...
{	
	...
	{	$data = array();
		...
		$data['users'] = $this->dashboard_m->users($config['per_page'],$this->uri->segment(3));
	...
	{	$this->load->library('form_validation');
		...
		if ($this->form_validation->run()) {
			$result = $this->dashboard_m->save_user();
			if ($result) {
	...
	public function single_user($id=null)
	{
		if ($id) {
			...
			$data['user'] = $this->dashboard_m->single_user($id);
	...
	public function edit_user($id=null)
	{
		if ($id) {
			...
			$data['user'] = $this->dashboard_m->edit_user($id);
	...
	public function update_user($id)
	{
		...
		if ($this->form_validation->run()) {
			$result = $this->dashboard_m->update_user($id);
			if ($result) {
	...
	{	$id=$this->session->userdata('id');
		if ($id) {
			...
			$this->form_validation->set_error_delimiters('<small class="error">','</small>');
			if ($this->form_validation->run()) {
				$result = $this->dashboard_m->update_password($id);
				if ($result) {
					$this->session->set_flashdata('success','User updated successfully');
					redirect('users');
				}
	...
	{	
		...
		if ($id) {
			$result=$this->dashboard_m->delete_user($id);
			if ($result==1) {

...
class Enrolled extends MY_Controller
{
	...
	{	$this->load->library('form_validation');
		...
		$student_id = $this->dashboard_m->find_enroll($student_id);
		...
		if ($this->form_validation->run()) {
			$result = $this->dashboard_m->save_enroll();
			if ($result) {
	...
	{
		if ($id) {
			$result=$this->dashboard_m->delete_enrolled($id,$batch_id);
			if ($result==1) {

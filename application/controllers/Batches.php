...
class Batches extends MY_Controller
{
	...
	{	$data = array();
		...
		$data['batches'] = $this->dashboard_m->batches($config['per_page'],$this->uri->segment(3));
	...
	{	$this->load->library('form_validation');
		...
		if ($this->form_validation->run()) {
			$result = $this->dashboard_m->save_batch();
			if ($result) {
	...
	public function single_batch($id=null)
	{
		if ($id) {
			...
			$data['batch'] = $this->dashboard_m->single_batch($id);
			$data['students'] = $this->dashboard_m->batch_enrolled($id);
	...
	public function edit_batch($id=null)
	{
		if ($id) {
			...
			$data['courses'] = $this->dashboard_m->courses();
			$data['batch'] = $this->dashboard_m->edit_batch($id);
	...
	public function update_batch($id)
	{
		...
		if ($this->form_validation->run()) {
			$result = $this->dashboard_m->update_batch($id);
			if ($result) {
	...
	{
		if ($id) {
			$result=$this->dashboard_m->delete_batch($id);
			if ($result==1) {
	...
	{	$data = array();
		...
		$results = $this->dashboard_m->ajax_batch($course_id);
		if ($results) {
			$select_batch = '';
